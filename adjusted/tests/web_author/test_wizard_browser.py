...
def main():
    ...
    project = {"project_id": "synthetic-wizard-a", "status": "configuring", "revision": 1,
               ...
               "content_profile": civs["content_profile"], "request": None, "progress": {"filled": None, "total": 1715}}
    ...
    def route(req):
        ...
        if path == "/api/start":
            ...
            posts.append(body)
            project.update(status="authoring", revision=2, request={k:v for k,v in body.items() if k != "expected_revision"})
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            assert "SDK" in page.locator("#agentHelp").inner_text()
            assert "不会自动启动" in page.locator("#agentHelp").inner_text()
...
def binding_preview():
    ...
    def route(request):
        ...
        if path=="/api/usage/bind-session":
            posts.append(request.request.post_data_json)
            usage["auto_capture"].update(bound_session_count=1,status="CONNECTED_PARTIAL",connection={"message":"已连接合成测试会话。","action":"wait_for_usage","action_label":"等待本轮真实用量记录。"})
            request.fulfill(json=usage); return
        target=WEB/("index.html" if path=="/" else path.lstrip("/"))
        if target.is_file() and target.resolve().is_relative_to(WEB.resolve()):
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            assert not posts
            page.locator("#bindSessionButton").click()
            page.wait_for_function("document.querySelector('#sessionBinding').classList.contains('hidden')")
            assert posts==[{"project_id":project_id,"expected_revision":7,"run_id":"synthetic-meter-run","agent":"codex","session_id":"synthetic-current-session-12345678"}],posts
            assert not errors,errors
            report={"status":"PASS","scope":"one synthetic binding POST and two desktop visual previews","real_projects_accessed":False,"binding_requests":posts,"page_errors":errors,"dimensions":dimensions}
