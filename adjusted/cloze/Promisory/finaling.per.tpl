...
(defrule
	(military-population >= {{FINAL_PRODUCTION_001}})
	(players-building-count any-enemy >= 1)
(or	(unit-type-count-total trebuchet-set < {{FINAL_PRODUCTION_002}})
	(population >= {{FINAL_PRODUCTION_003}}))
	(unit-type-count-total trebuchet-set < {{FINAL_PRODUCTION_004}})
	(can-train trebuchet)
=>
	(train trebuchet)
)
...
(defrule ;paphos ships
	(warboat-count >= 1)
	(strategic-number sn-dock-training-filter < 2)
(or	(wood-amount < 425)
	(gold-amount < 375))
(or	(building-type-count shipyard < 4)
	(strategic-number sn-dock-training-filter < 1))
=>
	(up-jump-rule 5))
(defrule
...
(defrule
	(up-research-status c: ri-heavy-incendiary-ship >= research-available)
...
(defrule
(or	(gold-amount >= {{FINAL_PRODUCTION_007}})
...
(defrule
(or	(gold-amount >= {{FINAL_PRODUCTION_008}})
	(unit-type-count-total 918 <= 0))
	(can-train monk)
=>
	(train monk)
...
(defrule
(or	(players-unit-type-count any-enemy battle-elephant-line >= {{FINAL_COUNTER_TRIGGER_009}})
(or	(players-unit-type-count any-enemy war-elephant-line >= {{FINAL_COUNTER_TRIGGER_010}})
(or	(players-unit-type-count any-enemy elephant-archer-line >= {{FINAL_COUNTER_TRIGGER_011}})
(or	(players-unit-type-count any-enemy ballista-elephant-line >= {{FINAL_COUNTER_TRIGGER_012}})
	(and	(food-amount >= {{FINAL_PRODUCTION_013}})
		(gold-amount >= {{FINAL_PRODUCTION_014}}))))))
	(can-train flaming-camel)
=>
	(train flaming-camel)
)
(defrule
	(food-amount >= {{FINAL_PRODUCTION_015}})
	(gold-amount >= {{FINAL_PRODUCTION_016}})
	(can-train petard)
=>
	(train petard)
)
...
(defrule
	(up-research-status c: ri-elite-hoplite >= research-available)
	(can-train hoplite)
=>
	(train hoplite)
)
...
(defrule
	(up-research-status c: ri-elite-phalangite >= research-available)
...
(defrule
	(up-research-status c: ri-elite-sannahya >= research-available)
...
(defrule
	(game-time < 60)
;	(game-time >= 2)
	(game-time > initial-delay)
	(building-count <= 0)
;	(building-type-count-total workshop < 1)
...
;	(fe-break-point 1 c:== 1 -1)
	...
	(up-set-target-object search-local c: 0)
	(up-get-point position-object br-temp-point-x)
	(up-get-object-data object-data-move-x gl-object2-x)
	(up-get-object-data object-data-move-y gl-object2-y)
)
...
(defrule
	...
	(up-object-data object-data-move-x g:== br-temp-point-x)
	(up-object-data object-data-move-y g:== br-temp-point-y)
=>
	(up-get-point position-map-size br-temp-point-x)
	(generate-random-number 1000)
	(up-get-fact random-number 0 temp-goal)
	(generate-random-number 1000)
	(up-get-fact random-number 0 temp-goal2)
	(up-modify-goal temp-goal g:mod br-temp-point-x)
	(up-modify-goal temp-goal2 g:mod br-temp-point-y)
	(up-modify-goal gl-object-x g:= temp-goal)
	(up-modify-goal gl-object-y g:= temp-goal2)
	(up-bound-point gl-object-x gl-object-x)
	(up-lerp-percent gl-object-x gl-center-x c: 25)
	(up-get-point position-object gl-object2-x)
	(up-lerp-percent gl-object-x gl-object2-x c: 75)
	(up-set-target-point gl-object-x)
	(up-target-point 0 action-move -1 -1)
)
;#end-if
...
(defrule
	(game-time > 91)
	(game-time < 550)
	(building-type-count-total workshop > 0)
	...
	(and(up-object-data object-data-move-x g:== br-temp-point-x)
	(up-object-data object-data-move-y g:== br-temp-point-y)))
;	(up-point-distance br-temp-point-x gl-object2-x < 7)
=>
	(up-get-point position-map-size br-temp-point-x)
	(generate-random-number 1000)
	(up-get-fact random-number 0 temp-goal)
