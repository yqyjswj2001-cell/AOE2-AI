...
def cursor_usage(workspace, sessions, started_at, home=None, environ=None):
    """Project-owned assistant bubble token fields, projected in read-only SQL.

    Zero-filled tokenCount objects are missing evidence, not measured free calls.
    Cache/reasoning remain unknown because this storage does not expose them.
    """
    home = Path.home() if home is None else Path(home)
    env = os.environ if environ is None else environ
    owned = {r["session_id"] for r in cursor_metadata(workspace, home, env)}
    bases = [Path(env.get("APPDATA") or home / "AppData/Roaming") / "Cursor/User",
             home / "Library/Application Support/Cursor/User", home / ".config/Cursor/User"]
    result = []
    for base in bases:
        ...
        try:
            with readonly_db(path) as db:
                for session in sessions:
                    if session not in owned:
                        continue
                    prefix = "bubbleId:" + session + ":"
                    # Range lookup uses the key index; LIKE would scan the full store.
                    rows = db.execute("SELECT key,json_extract(value,'$.createdAt'), "
                        "json_extract(value,'$.tokenCount.inputTokens'), "
                        "json_extract(value,'$.tokenCount.outputTokens'), "
                        "json_extract(value,'$.modelInfo.modelName') FROM cursorDiskKV "
                        "WHERE key>=? AND key<? AND json_extract(value,'$.type')=2", (prefix, prefix + "~"))
                    for key, created, inp, out, model in rows:
                        when = timestamp(created)
                        if when is not None and when < started_at:
                            continue
                        usage = None
                        if type(inp) is int and type(out) is int and min(inp, out) >= 0 and inp + out > 0:
                            usage = {"input_tokens": inp, "output_tokens": out, "total_tokens": inp + out,
                                     "cached_input_tokens": None, "cache_write_tokens": None,
                                     "reasoning_output_tokens": None}
                        result.append({"agent": "cursor", "session": session, "key": key,
                            "timestamp": when, "model": model or "unknown", "usage": usage,
                            "snapshot": True, "gap": "CURSOR_USAGE_NOT_REPORTED" if usage is None else None})
        except sqlite3.Error:
            continue
    return result
