...
_ROWS = [
    ...
    ("cursor", "Cursor", "explicit_import", "Cursor IDE 本机 tokenCount 不可靠，不作为真实用量来源。Cursor SDK 可导入最终 RunResult.usage / getUsage()。", "Cursor IDE 不自动读取 token；不要按上下文占用估算。若本轮通过 Cursor SDK 运行，导入该 run 的真实 usage；否则保持未采集。"),
