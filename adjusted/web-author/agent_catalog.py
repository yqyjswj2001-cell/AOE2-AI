...
_ROWS = [
    ...
    ("cursor", "Cursor", "conditional_local", "绑定 composer 后读取本机气泡 tokenCount；全零或缺失保持未采集。也支持官方 SDK 最终 usage 导入。", "IDE 上下文占用不能计费。导入本轮 SDK RunResult.usage，或提供可证明归属的逐次 usage。"),
