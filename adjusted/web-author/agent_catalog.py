...
_ROWS = [
    ...
    ("cursor", "Cursor", "explicit_import", "项目 Hook 记录当前 conversation，并用 stop 事件里的父代理 token 入账。", "授权后由 stop Hook 自动计入本轮父代理 token，无需 Team Admin API。子代理不在该计数内。同一 conversation 已有 Hook 合计时，不再叠加 Admin Usage Events。"),
